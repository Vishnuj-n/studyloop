...
func localFlashcardStateToCard(state models.FlashcardState, dueAt, lastReviewedAt int64) fsrs.Card {
...
func computeNextState(state models.FlashcardState, rating int, now time.Time, dueAt, lastReviewedAt int64) (models.FlashcardState, error) {
	...
	fsrsCard := localFlashcardStateToCard(state, dueAt, lastReviewedAt)
