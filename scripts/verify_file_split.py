...
def extract_top_level_declarations(code: str) -> Dict[str, str]:
    ...
    decl_pattern = re.compile(
        r"^(func\s*(?:\([^)]+\)\s*)?([A-Za-z0-9_]+)|type\s+([A-Za-z0-9_]+)|const\s+(?:\(|([A-Za-z0-9_]+))|var\s+(?:\(|([A-Za-z0-9_]+)))"
    )
    ...
    while i < n:
        ...
        if match:
            ...
            func_match = re.match(r"^func\s*(\([^)]+\)\s*)?([A-Za-z0-9_]+)", line)
            type_match = re.match(r"^type\s+([A-Za-z0-9_]+)", line)
            const_match = re.match(r"^const\s*(?:\(\s*)?([A-Za-z0-9_]*)", line)
            var_match = re.match(r"^var\s*(?:\(\s*)?([A-Za-z0-9_]*)", line)
            ...
            while i < n:
                ...
                # Simple count ignoring string/comment contents for standard formatted code
                # (Assuming gofmt-compliant code)
                ...
                i += 1
                if brace_count == 0 and paren_count == 0 and i > start_i:
                    break
...
def verify_split(original_code: str, split_codes: List[Tuple[str, str]], strict: bool = False) -> bool:
    ...
    if not errors_found and not added_in_splits:
        ...
        print(f"\n[FAILED] Detected {len(missing_in_splits)} missing and {modified_count} modified declarations.")
        return False
